...
class Flight(object):
    ...
    def simulate(self):
        ...
        acceleration_now = Formula.vector_addition([acceleration_x_now, acceleration_y_now])
...
def run_sim(sim_flight, flight_name, max_step, sim_time_step):
    ...
    if not os.path.exists("./Results/{}".format(flight_name)):
        os.makedirs("./Results/{}".format(flight_name))
...
def main():
    ...
    sim_data = Data(data_file="./Results/{}/Results_Data.csv".format(sim_flight_name))
    ...
    count = 1
    sim_opt_max_step = 40
    height_diff = abs(sim_flight_a.get_max_height() - sim_flight_b.get_max_height())
    abort_condition = 10   # Minimal difference seeked in heigtdiff
    while count <= sim_opt_max_step and not (height_diff < abort_condition):
        sim_flight_name = "A150_TankSetup_{}".format(divisor)
        sim_data = Data(data_file="./Results/{}/Results_Data.csv".format(sim_flight_name))
        A150_mass_propellant_tank_two = A150_mass_propellant - A150_mass_propellant/divisor
        sim_data.name = "Stufe 1: {0:.2f} kg Stufe 2: {1:.2f} kg".format(A150_mass_propellant_tank_two, A150_mass_propellant/divisor)
        A150 = Rocket(pos=[earth_radius, 0.0], velocity=[0.0, 0.0], acceleration=[0.0, 0.0])
        A150_liquid_tank_one_split = Tank(mass_part=116.1216, surface_part=0.0, drag_coefficient_part=0.0,
                                    mass_propellant=A150_mass_propellant/divisor, mass_change_tank=9.394,
                                    velocity_exhaust_tank=1417.32, surface_nozzle=0.0275, pressure_nozzle=101325.0)
        ...
        # Evaluating next flight_p
        diff_a = sim_flight_a.get_max_height() - sim_flight_p.get_max_height()
        diff_b = sim_flight_b.get_max_height() - sim_flight_p.get_max_height()
        if diff_a < diff_b:
            sim_flight_b = sim_flight_p
            divisor = divisor + divisor/2
        elif diff_a == diff_b:
            break
        else:
            sim_flight_a = sim_flight_p
            divisor = divisor - divisor/2
        height_diff = abs(sim_flight_a.get_max_height() - sim_flight_b.get_max_height())
        count += 1
